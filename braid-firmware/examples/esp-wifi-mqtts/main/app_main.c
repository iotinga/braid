...
static void send_binary(esp_mqtt_client_handle_t client) {
    ...
    // sending only the configured portion of the partition (if it's less than
    // the partition size)
    int binary_size = MIN(CONFIG_BROKER_BIN_SIZE_TO_SEND, partition->size);
    int msg_id = esp_mqtt_client_publish(client, "/topic/binary",
                                         binary_address, binary_size, 0, 0);
...
static void mqtt_app_start(void) {
    // char *dev_cert_buf = (char *)calloc(1536, sizeof(char));
    // atecc_read_cert(CERT_TYPE_DEVICE, dev_cert_buf, 1536 * sizeof(char));
    // ESP_LOGI(TAG, "%s", dev_cert_buf);
