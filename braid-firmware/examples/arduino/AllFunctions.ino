...
void setup() {
    ...
    // Starting the machine requires at least 1 second of low level, and with a
    // level conversion, the levels are opposite
    delay(1000);
...
void loop() {
    ...
    * * */
     ...
     * <mode> "CAT-M"   "NB-IOT"
     * <band>  The value of <band> must is in the band list of getting from
     * AT+CBANDCFG=? For example, my SIM card carrier "NB-iot" supports B8.  I
     * will configure +CBANDCFG= "Nb-iot ",8
     */
                ...
                if (c == ',') {
                    if (input.indexOf(": ") >= 0) {
                        ...
                        if (data.toInt() > 0 && data.toInt() < 25) {
                            modem.sendAT("+CGDCONT=" + String(data.toInt()) +
                                         ",\"IP\",\"" + String(apn) +
                                         "\",\"0.0.0.0\",0,0,0,0");
                        }
                        input = "";
