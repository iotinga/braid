...
def encode_dates(nvb_time, expire_years) -> bytes:
...
def sign_csr(cert_sign_req, ca_cert, ca_privkey, device_sn, nva_years) -> bytes:
    ...
    device_subject = x509.Name(
        [
            x509.NameAttribute(
                NameOID.ORGANIZATION_NAME, ca_cert.subject.get_attributes_for_oid(NameOID.ORGANIZATION_NAME)[0].value
            ),
    ...
    device_cert = (
        ...
        .not_valid_before(nvb_time)
        .not_valid_after(nva_time)
        .add_extension(
            x509.KeyUsage(
        ...
        .add_extension(x509.AuthorityKeyIdentifier.from_issuer_public_key(ca_privkey.public_key()), critical=False)
        .sign(private_key=ca_privkey, algorithm=hashes.SHA256(), backend=default_backend())
    )
