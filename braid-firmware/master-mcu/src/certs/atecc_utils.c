...
static const char *TAG = "certs/atecc_utils";
...
static const uint8_t public_key_x509_header[] = {0x30, 0x59, 0x30, 0x13, 0x06, 0x07, 0x2A, 0x86, 0x48,
                                                 0xCE, 0x3D, 0x02, 0x01, 0x06, 0x08, 0x2A, 0x86, 0x48,
                                                 0xCE, 0x3D, 0x03, 0x01, 0x07, 0x03, 0x42, 0x00, 0x04};

int convert_pem_to_der(const unsigned char *input, size_t input_len, unsigned char *output, size_t *output_len) {
    int ret;
    ...
    if (s1 == NULL) {
        ESP_LOGE(TAG, "input (of size %d) does not contain 'BEGIN': %s", input_len, input);
        return -1;
    }

    s2 = (unsigned char *)strstr((const char *)input, "-----END");
    if (s2 == NULL) {
        ESP_LOGE(TAG, "input (of size %d) does not contain 'END': %s", input_len, input);
        return -1;
    }
    ...
    if (s2 > end) {
        ...
        ESP_LOGE(TAG, "%s", s1);
        ESP_LOGE(TAG, "%s", s2);
        ESP_LOGE(TAG, "END is out of bounds (%d > %d)", s2 - input, end - input);
        return -1;
    }
...
void convert_der_to_pem(uint8_t *der, size_t der_len, char *pem, size_t pem_max_len) {
...
esp_err_t init_atecc608a(char *device_type, uint8_t i2c_sda_pin, uint8_t i2c_scl_pin, uint8_t do_lock, int *err_ret) {
    int ret = 0;
    ...
    if (do_lock == 1) {
        ...
        if (!is_zone_locked) {
            ...
            if (ret != ATCA_SUCCESS) {
                ESP_LOGE(TAG, "error in locking config zone, ret = 0x%02x", ret);
                goto exit;
            }
        ...
        if (!is_zone_locked) {
            ...
            if (ret != ATCA_SUCCESS) {
                ESP_LOGE(TAG, "\nerror in locking data zone, ret = 0x%02x", ret);
                goto exit;
            }
...
esp_err_t atecc_print_info(uint8_t *serial_no, int *err_ret) {
    ...
    if (ATCA_SUCCESS != (ret = atcab_info(rev_info))) {
        ESP_LOGE(TAG, "Error in reading revision information, ret is 0x%02x", ret);
        goto exit;
    }
...
esp_err_t receive_cert_def(unsigned char *cert_def_array, size_t data_len, cert_type_t cert_type) {
    ...
    memset(cert_def_array, 0, data_len);
    ESP_LOGD(TAG, "Reading %d bytes", data_len);
    SerialRead(cert_def_array, data_len);
...
esp_err_t atecc_input_cert(unsigned char *cert_buf, size_t cert_len, cert_type_t cert_type, bool lock, int *err_ret) {
    int ret = -1;
    uint8_t der_cert[cert_len];
    size_t der_cert_size = cert_len;
    ...
    ESP_LOGD(TAG, "Reading %d bytes", cert_len);
    SerialRead(cert_buf, cert_len);
    ...
    if (cert_type == CERT_TYPE_DEVICE) {
        ECU_DEBUG_LOG(TAG, "Writing device cert to secure element");
        if (ATCA_SUCCESS !=
            (ret = atcacert_write_cert((const atcacert_def_t *)&g_cert_def_common, der_cert, der_cert_size + 1))) {
            ESP_LOGE(
                TAG, "writecert failed, ret is 0x%02x\nPlease make sure that the device cert slot is not locked", ret);
            goto exit;
        }
        if (lock) {
            ...
            if (ret != ATCA_SUCCESS) {
                ESP_LOGE(TAG,
                         "Failed to lock slot %d (device certificate)\nThis action is supposed to fail if the "
                         "slot is already locked",
                         ATECC608A_DEVICE_CERT_SLOT);
                goto exit;
            }
        ...
        ECU_DEBUG_LOG(TAG, "Writing signer cert to secure element");
        ...
        if (lock) {
            ...
            if (ret != ATCA_SUCCESS) {
                ESP_LOGE(TAG,
                         "Failed to lock slot 12 (signer certificate),\nThis action is supposed to fail if the "
                         "slot is already locked");
                goto exit;
            }
            ESP_LOGI(TAG, "Slot %d has been locked, and cannot be used again", ATECC608A_DEVICE_CERT_SLOT);
...
esp_err_t atecc_read_cert(cert_type_t cert_type, char *pem, size_t pem_max_len) {
